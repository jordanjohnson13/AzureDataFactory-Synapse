param (
    [string]$SubscriptionName = ##enteryoursubscriptionnamehere##,
    [bool]$debug = $false
)
    ...
    if (!$isServerSynapseWorkspace) {
        ...
        foreach ($SynapseSqlPool in $allSynapseSqlPools) {
            ...
            elseif ($SynapseSqlPool.Status -eq "Paused") {
                ...
                $startTimeResume = Get-Date
                Write-Output  "  -> Resuming Synapse SQL Pool [$($SynapseSqlPool.DatabaseName)]"
                ...
                if ($resultsynapseSqlPool.Status -eq "Online") {
                    Write-Output "  -> Synapse SQL Pool [$($resultsynapseSqlPool.DatabaseName)] resumed in $($durationResume.Hours) hours, $($durationResume.Minutes) minutes and $($durationResume.Seconds) seconds. Current status [$($resultsynapseSqlPool.Status)]"
                }
                else {
                    if (!$debug) {
                        $iErrorCount += 1;
                        Write-Error "  -> (resultsynapseSqlPool.Status -eq ""Online"") Synapse SQL Pool [$($resultsynapseSqlPool.DatabaseName)] online in $($durationResume.Hours) hours, $($durationResume.Minutes) minutes and $($durationResume.Seconds) seconds. Current status [$($resultsynapseSqlPool.Status)]"
                    }
                    else {
                        Write-Host "This is a debug session - Nothing was done" -ForegroundColor Yellow
                    }
            ...
            ##########################################################################################################################################################
                ...
                $iErrorCount += 1;
                Write-Error "  -> ((SynapseSqlPool.Status -eq ""Paused"")) Checking Synapse SQL Pool [$($SynapseSqlPool.DatabaseName)] found with status [$($SynapseSqlPool.Status)]"
            }
            ##########################################################################################################################################################
...
##########################################################################################################################################################
if ($iErrorCount > 0) {
    Write-Error -Message "Resume DB script error count ($($iErrorCount)) check logs" `
